...
class FixFragmentNavigator(
    ...
    override fun navigate(
        ...
        isAdded = if (initialNavigation) {
            ...
            if (mBackStack.size > 1) {
                ...
                setSupFieldValue("mIsPendingBackStackOperation", true)
            }
            false
        } else {
    ...
    private fun setSupFieldValue(fieldName: String, newValue: Boolean) {
        try {
            val field =
                FixFragmentNavigator::class.java.superclass!!.getDeclaredField(fieldName)
            field.isAccessible = true
            field[this] = newValue
        } catch (e: NoSuchFieldException) {
            e.printStackTrace()
